...
disp("D_effective = Depth_at_face- clear_cover-dia/2")
D_effective = Depth_at_face- clear_cover-dia/2;
...
Depth=Depth_at_face-x;
disp("Depth=Depth_at_face-x")
printf("Depth = % d mm \n",Depth)
...
Punching_capacity = (.25*sqrt(fck)*Depth_effective*4*(Side_of_column+(Depth_effective/2)))/(1000*1.5);
disp("Punching_capacity = (.25*sqrt(fck)*Depth_effective*4*(Side_of_column+(Depth_effective/2)))/(1000*1.5)")
